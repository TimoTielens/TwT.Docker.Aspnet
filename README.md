...
This container itself doesn't do that much itself because there is no code deployed to it yet. See the dockerfile below as an example how this image can be used
    ...
    FROM --platform=$BUILDPLATFORM mcr.microsoft.com/dotnet/sdk:8.0.13 AS build
    ARG TARGETARCH
    WORKDIR /source
    ...
    # final stage/image
    FROM timotielens/aspnet:8.0.13
    EXPOSE 8080 2222
    WORKDIR /app
    COPY --from=build /app .
    USER $APP_UID
...
3. Run the following command to build the container **docker build --build-arg BUILD_DATE=2025-02-26 -t timotielens/aspnet:8.0.11 .**
